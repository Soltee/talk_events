...
@section('content')
    ...
    <div class="w-full flex   px-6 md:px-24  lg:px-40  my-8">
        ...
        <div class="flex-1 md:pl-8 flex flex-col">
...
{{-- 			flex flex-wrap justify-between
 --}}			">
	        			...
	        			<div class="img-hover-zoom">
	        				<div class="relative">
		        				<a class="" href="{{ url('events', $event->id . '-' . $event->slug())}}">
			        				<img  class="w-full mb-5 rounded " src="{{ asset($event->cover) }}" alt="{{ $event->slug() }}">
			        			</a>
			        		
			        		</div>
		        		</div>
	        			<div class="flex flex-col items-start justify-between">
	        				<h5 class="mb-3 text-lg text-c-dark-gray">{{ $event->name }}</h5>
	        			</div>
	        ...
	        {{-- {{ $events->appends(request()->input())->nextPageUrl() }} --}}
