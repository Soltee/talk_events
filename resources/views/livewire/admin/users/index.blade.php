...
<div class="">
	...
	<!-- Add User Icon -->
	@can('add users')
		<a href="{{ route('user.create') }}" class="fixed right-0 bottom-0 mr-32 mb-32 text-xl font-3xl text-white bg-blue-600 rounded-full px-6 py-5  hover:opacity-75 z-10">+</a>
	@endcan
							    ...
							    <td class="px-5 whitespace-no-wrap py-5 border-b border-gray-200 bg-white text-sm">
							        @if($user->avatar)
							            <a class="text-blue-500 hover:underline" href="{{ route('user.show', $user->id) }}">
							                    <img class="w-24 h-24 hover:opacity-75 rounded-lg object-cover object-center" src="{{ $user->avatar }}" onerror="this.src='/images/placeholder.png'">
							            </a>
							        @else
